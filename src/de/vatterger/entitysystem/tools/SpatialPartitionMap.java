...
public class SpatialPartitionMap<T> {
	...
	Bag<Bag<Bucket<T>>> buckets = new Bag<Bag<Bucket<T>>>(16);
	int cellsize;
	
	public SpatialPartitionMap(int cellsize) {
		if(cellsize > 0) {
	...
	public void clear() {
		Profiler p = new Profiler("SpartialMap.clear", ProfileUnit.MILLISECONDS);
