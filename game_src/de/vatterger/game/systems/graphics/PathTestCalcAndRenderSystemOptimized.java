...
public class PathTestCalcAndRenderSystemOptimized extends BaseSystem {
	...
	float[]	nx = new float[2048];		// y
	float[]	ny = new float[2048];		// x
	float[]	nr = new float[2048];		// radius
	
	float[]	ndx = new float[2048];		// dir-x
	float[]	ndy = new float[2048];		// dir-y

	float[]	nc = new float[2048];		// cost

	int[]	np = new int[2048];			// parent
	...
	private static boolean overlaps(float x1,float y1, float r1, float x2,float y2, float r2) {
		float dx = x2 - x1;
		float dy = y2 - y1;
