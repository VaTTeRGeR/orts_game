...
public class IntArrayTimSort {
     ...
     */
    private IntArrayTimSort(int[] a, IntBinaryOperator c, int[] work, int workBase, int workLen) {
        this.a = a;
        this.c = c;
        ...
        if (work == null || workLen < tlen || workBase + tlen > work.length) {
            ...
            System.out.println("Resized TimSort Array: " + tlen);
     ...
     * @since 1.8
     */
    public static void sort(int[] a, int lo, int hi, IntBinaryOperator c,
    ...
    private static int countRunAndMakeAscending(int[] a, int lo, int hi,
        ...
        if (c.applyAsInt(a[runHi++], a[lo]) < 0) { // Descending
            while (runHi < hi && c.applyAsInt(a[runHi], a[runHi - 1]) < 0)
                runHi++;
    ...
    private static int gallopLeft(int key, int[] a, int base, int len, int hint,
        ...
        int lastOfs = 0;
        int ofs = 1;
        if (c.applyAsInt(key, a[base + hint]) > 0) {
            // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
            int maxOfs = len - hint;
            while (ofs < maxOfs && c.applyAsInt(key, a[base + hint + ofs]) > 0) {
                lastOfs = ofs;
                ofs = (ofs << 1) + 1;
        ...
        while (lastOfs < ofs) {
            ...
            if (c.applyAsInt(key, a[base + m]) > 0)
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        ...
        IntBinaryOperator c = this.c;  // Use local variable for performance
    ...
    outer:
        while (true) {
            ...
            do {
                ...
                if (c.applyAsInt(a[cursor2], tmp[cursor1]) < 0) {
                    a[dest++] = a[cursor2++];
                    count2++;
                    count1 = 0;
                    if (--len2 == 0)
    ...
    outer:
        while (true) {
            ...
            do {
                ...
                if (c.applyAsInt(tmp[cursor2], a[cursor1]) < 0) {
                    a[dest--] = a[cursor1--];
                    count1++;
                    count2 = 0;
                    if (--len1 == 0)
    ...
    private int[] ensureCapacity(int minCapacity) {
        if (tmpLen < minCapacity) {
            ...
            System.out.println("Resized TimSort Array: " + newSize);
