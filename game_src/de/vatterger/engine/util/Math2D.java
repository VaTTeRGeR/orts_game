...
public final class Math2D {
	...
	public static float limitAngle (float angle, float min, float max) {
		...
		// Make sure min <= max to enable midpoint shift
		if (max < min) {
			min -= 360f;
		}

		// Calculate midpoint of min...max range
		float centerOffset = ( min + max ) * 0.5f;
		
		// Transform center of min...max range to zero
		angle -= centerOffset;
		min -= centerOffset;
		max -= centerOffset;
		
		// Transform -360...+360 => -180...+180
		if(angle > 180f) {
			angle -= 360f;
		} else if(angle < -180f) {
			angle += 360f;
		...
		return normalize_360(angle + centerOffset);
	}
}
